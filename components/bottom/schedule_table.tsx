...
export function ScheduleTable({ selectedDate, onRowDoubleClick }) {
  ...
  switch (option) {
    case yesterdayDate:
      return {
        ...
        data: [
          { id: 'S0001', status: "Draft", date: yesterdayDate, cngVolumeDelivered: 642, totalCost: 2000000 },
          { id: 'S0002', status: "Optimized", date: yesterdayDate, cngVolumeDelivered: 601, totalCost: 1400000 },
          { id: 'S0003', status: "Optimized", date: yesterdayDate, cngVolumeDelivered: 620, totalCost: 1500000 },
          { id: 'S0004', status: "Ready", date: yesterdayDate, cngVolumeDelivered: 642, totalCost: 1850000 }
        ]
      };
  ...
  const tableConfig = useMemo(() => getTableConfig(selectedDate), [selectedDate]);
  ...
  if (tableConfig.columns.length === 0) {
    return (
      <div className="flex flex-col h-full w-full bg-gray-100 items-center justify-center">
        <div className="text-gray-500 text-lg">No table configuration available for: {selectedDate}</div>
      </div>
    );
  }
        ...
        <table className="min-w-full divide-y divide-brand-F1EDEA">
          <thead className="bg-brand-F1EDEA sticky top-0">
            <tr>
              {tableConfig.columns.map((column) => (
                <th
                  key={column.key}
                  className={`py-1 text-center text-sm font-bold text-black-500 uppercase ${column.width}`}
                >
                  {column.label}
                </th>
              ))}
          ...
          <tbody className="bg-brand-F1EDEA divide-y divide-brand-F1EDEA">
            {tableConfig.data.map((row, index) => (
              <tr key={index} className="hover:bg-gray-50 transition-colors cursor-pointer" onDoubleClick={() => handleRowDoubleClick(row)}>
                {tableConfig.columns.map((column) => (
                  <td key={column.key} className="py-1 text-center whitespace-normal text-sm text-black">
